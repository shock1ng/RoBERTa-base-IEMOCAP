# -*- coding: utf-8 -*-
# @Time : 2023/10/19 13:06
# @Author : JohnnyYuan
# @File : utils_5_mel_bert_w2v2.py
# -*- coding: utf-8 -*-
...
class subDataset(Dataset.Dataset):
    ...
    def __getitem__(self, item):
        ...
        ids = torch.Tensor(self.Ids_1[item])
        att = torch.Tensor(self.Att_1[item])
        return data_1, label ,ids ,att
...
def Feature(data, args):
    ...
    for i in range(len(data)):
        input_data_w2v2.append(data[i]['wav_encodings'])  # 此处拿取的是经过wav2vec2后的数据

    input_data_spec = []
    for i in range(len(data)):
        input_data_spec.append(data[i]['spec_data'])
    ...
    for i in range(len(input_data_spec)):
        ...
        ha = ha.reshape((1, 300, 128))  # 把(300,80)变成(1,300,80)
    ...
    return input_data_spec_CNN, input_data_w2v2, input_label, input_data_id, input_label_org ,input_ids, input_att_mask
...
def Get_data(data, train, test, args):  # test 从[0,1,2,3,4]里抽一个，train则取剩下的，都是列表格式
    ...
    input_train_data_spec, input_train_data_spec_w2v2, input_train_label, input_train_data_id, _ ,input_train_ids, input_train_att= Feature(train_data, args)
    input_test_data_spec, input_test_data_spec_w2v2, input_test_label, input_test_data_id, input_test_label_org, input_test_ids, input_test_att= Feature(test_data, args)
    ...
    train_dataset = subDataset(input_train_data_spec, label, input_train_ids, input_train_att)
    test_dataset = subDataset(input_test_data_spec, label_test, input_test_ids, input_test_att)
