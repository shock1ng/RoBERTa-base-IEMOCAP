...
data_save = "RoBERT_Only.txt"

parser = argparse.ArgumentParser(description="RoBERT_Model")
...
def Train(epoch):
    ...
    total_samples = 0
    correct_samples = 0
    for batch_idx, (_, target, id) in enumerate(train_loader):
        id = list(id)
        # 此处加载的data是torch.Size([128, 1, 300, 80])，也就是128个单通道300*80的矩阵， 加载的id是128个元素的列表
        if args.cuda:
            target  = target.cuda()
            # print("已启用GPU训练")
...
def Test():
    ...
    label_true = []
    with torch.no_grad():
        for batch_idx, (_, target ,id) in enumerate(test_loader):
            if args.cuda:
                target = target.cuda()
            target = Variable(target)
            utt_optim.zero_grad()
            # data_1 = torch.squeeze()
            model_out = cnet(id, args)
...
for index, (train, test) in enumerate(kf.split(data)):
    ...
    utt_optim = optim.AdamW(cnet.parameters(), lr=lr)  #默认ADAM,改SGD看看
    f1 = 0
    recall = 0
    recall_list = []
    f1_list = []
